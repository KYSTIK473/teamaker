...
CustomStepper::CustomStepper(PCF8574 & pcf_, byte pin1, byte pin2, byte pin3, byte pin4, byte steps[], float spr, float rpm, byte direction)
  : pcf(pcf_)
{
  this->setRPM(rpm);
  this->setSPR(spr);
  ...
  this->pin3 = pin3;
  this->pin4 = pin4;
  pinMode(pcf, this->pin1, OUTPUT);
  pinMode(pcf, this->pin2, OUTPUT);
  pinMode(pcf, this->pin3, OUTPUT);
  pinMode(pcf, this->pin4, OUTPUT);
...
void CustomStepper::step()
{
  ...
  if (this->stepsToGo - this->stepsGone > 0)
  {
    ...
    {
      digitalWrite(pcf, this->pin1, (this->steps[this->nextStep] & B1000));
      digitalWrite(pcf, this->pin2, (this->steps[this->nextStep] & B0100));
      digitalWrite(pcf, this->pin3, (this->steps[this->nextStep] & B0010));
      digitalWrite(pcf, this->pin4, (this->steps[this->nextStep] & B0001));
	  this->setTimer();
...
{
  this->time = 0;
  digitalWrite(pcf, this->pin1, LOW);
  digitalWrite(pcf, this->pin2, LOW);
  digitalWrite(pcf, this->pin3, LOW);
  digitalWrite(pcf, this->pin4, LOW);
  this->done = true;
}
