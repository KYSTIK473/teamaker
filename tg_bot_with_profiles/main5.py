...
async def choosing(message: types.Message):
    ...
    if message.text == 'Заварить чай':
        ...
        kb = [
            [
                types.KeyboardButton(text="Чёрный королевский")],
            [types.KeyboardButton(text="Зелёный со слоном")],
            [types.KeyboardButton(text="Что-то еще (Coming soon)")
             ],
        ]
        keyboard = types.ReplyKeyboardMarkup(
...
async def profile_tea(message: types.Message):
    ...
    if message.text == 'Чёрный королевский':
        await message.answer('Вкус чёрного чая лишён горечи, настой имеет оранжевый или красный цвет. Чай обладает сложным действием, одновременно тонизирующим и успокаивающим.',
                             reply_markup=types.ReplyKeyboardRemove())
    else:
        await message.answer('зелёный',
                             reply_markup=types.ReplyKeyboardRemove())
